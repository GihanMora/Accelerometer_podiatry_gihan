...
def evaluate_regression_modal(model_b, REG_RESULTS_FOLDER, data_dict, TIME_PERIODS, group):
    ...
    out_col_names = ['participant', 'R2_Overall', 'R2_SB', 'R2_LPA', 'R2_MVPA', 'Pearson_Overall', 'Pearson_SB', 'Pearson_LPA', 'Pearson_SBLPA', 'Pearson_MVPA',
    ...
    for key, data in tqdm(data_dict.items(), desc='Predicting'):
        ...
        X_test_sblpa, y_test_sblpa = [X_test[i] for i, c in enumerate(y_class) if (c == 1 or c == 2)], [y_test[i] for i, c in enumerate(y_class) if (c == 1 or c == 2)]
        if len(y_test_sblpa) > 0:
            p_sblpa, r2_sblpa, _ = calculate_error(X_test_sblpa, y_test_sblpa, model_b)
        else:
            p_sblpa, r2_sblpa = 0, 0
        ...
        result_row = [key, r2_overall, r2_sb, r2_lpa, r2_mvpa, p_overall, p_sb, p_lpa, p_sblpa, p_mvpa,
